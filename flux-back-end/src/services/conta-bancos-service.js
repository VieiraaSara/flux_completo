...
class ContaBancosService {
    ...
    try {
      ...
      if (!id_conta_bancaria_destino) {
        ...
        if (!saldoContaAtualizado || !saldoContaAtualizado.data) {
          await t.rollback();
          return {
            message: saldoContaAtualizado.message,
            status: saldoContaAtualizado.status,
          };
        }

        const novoSaldo = saldoContaAtualizado.data.saldo;
        const contaAtualizada = await contaBancariaRepository.put(
          conta_bancaria_origem_id,
          novoSaldo,
          fkUsuarioId
        );
        ...
        const contaBancariaUpdate = await contaBancariaRepository.findOne({
  ...
  static async buscarContaBancaria(contaBancaria_id, usuario_id) {
    const conta = await contaBancariaRepository.findOne({
      contaBancaria_id,
      usuario_id,
    });
    if (!conta) {

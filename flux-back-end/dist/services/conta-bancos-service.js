...
class ContaBancosService {
    static buscarContaBancaria(contaBancaria_id, usuario_id) {
        return __awaiter(this, void 0, void 0, function* () {
            const conta = yield contaBancosrepository.findOne({
                contaBancaria_id,
                usuario_id,
            });
            if (!conta) {
    ...
    static verificarSaldoSuficiente(contaBancaria_id, usuario_id, valor) {
        return __awaiter(this, void 0, void 0, function* () {
            const conta_id = contaBancaria_id;
            const conta = yield ContaBancosRepository.findOne({ conta_id, usuario_id });
            const saldoDisponivelNaConta = yield conta.data.Contum.saldo;
    ...
    static atualizarSaldoConta(idConta, novoSaldo) {
        return __awaiter(this, void 0, void 0, function* () {
            yield contaBancariaRepository.update({ saldo: novoSaldo }, { where: { id: idConta } });
        });
    }
    static registrarTransacao(dadosTransacao) {
        return __awaiter(this, void 0, void 0, function* () {
            console.log('--------------------------------------------');
            console.log(dadosTransacao);
            console.log('--------------------------------------------');
            const res = yield Transacao.create(dadosTransacao);
            return { data: res };
        });
    }
}
_a = ContaBancosService;
ContaBancosService.realizarTransferencia = (conta_bancaria_origem_id, valor_transferencia, fkUsuarioId, descricao_transacao, id_conta_bancaria_destino) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const contaBancaria = yield _a.buscarContaBancaria(conta_bancaria_origem_id, fkUsuarioId);
        if (!contaBancaria || !contaBancaria.data) {
            yield t.rollback();
            return { message: contaBancaria.message, status: contaBancaria.status };
        }
        const contaDestino = yield _a.buscarContaBancaria(id_conta_bancaria_destino, fkUsuarioId);
        if (!contaDestino) {
            yield t.rollback();
        ...
        if (contaDestino.data.Pix.status === "VALIDANDO") {
            console.log("Não é possivel realizar transferências se sua chave pix não está registrada");
            return {
                message: "Não é possivel realizar transferências se sua chave pix não está registrada",
                status: 400,
            };
        }
        if (contaDestino.data.id_contaBancos === contaBancaria.data.id_contaBancos) {
            ...
            return {
                message: "Você não pode realizar uma transferência para uma mesma conta bancária",
                status: 400,
            };
        }
        yield Promise.all([
            contaBancariaRepository.put(contaBancaria.data.Contum.id_conta, novoSaldoOrigem, contaBancaria.data.usuario_id, { transaction: t }),
            contaBancariaRepository.put(contaDestino.data.Contum.id_conta, novoSaldoDestino, contaDestino.data.usuario_id, { transaction: t }),
        ]);
        yield _a.registrarTransacao({
        ...
        yield t.commit();
        const contaBancariaUpdate = yield contaBancosrepository.findOne({
...
ContaBancosService.buscarContasDoFlux = (fkUsuarioId) => __awaiter(void 0, void 0, void 0, function* () {
    console.log("SERVICE", fkUsuarioId);
    const res = yield contaBancosrepository.get(fkUsuarioId);
    if (!res) {
